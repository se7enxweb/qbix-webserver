...
class Q_WebServer_Panel_Store
{
	...
	//
	// Kept in acl/panel.json, beside the password hash, under the same lock
	// and the same trust rule, and never returned to any request but the
	// one-time enrollment view. The shape:
	//
	//   twofactor        { secret (base32), enabled (bool),
	//                      recovery [sha256 hex...], lastStep (int) }
	//   twofactorPending { secret (base32), created (int) }
	//
	// 'secret' is the shared secret an authenticator holds; 'recovery' holds
	// only the HASHES of the one-time recovery codes (the plaintext is shown
	// once, at enrollment, and never stored); 'lastStep' is the newest TOTP
	// step already accepted, so a code cannot be replayed. 'twofactorPending'
	// is a secret generated by begin-enrollment but not yet proven with a
	// code, so it can never sign anything in until confirm-enrollment moves it
	// across.

	/** The active two-factor record, or an empty array (also when locked). */
	static function twoFactorGet()
	{
		$c = self::aclLoad();
		return is_array($c['twofactor'] ?? null) ? $c['twofactor'] : array();
	}

	/** Whether two-factor is enrolled and switched on for the panel. */
	static function twoFactorEnabled()
	{
		$t = self::twoFactorGet();
		return !empty($t['enabled']) and !empty($t['secret']);
	}

	/** Change the active two-factor record in place, under the lock. */
	static function twoFactorUpdate(callable $fn)
	{
		return self::aclUpdate(function (array $c) use ($fn) {
			$t = is_array($c['twofactor'] ?? null) ? $c['twofactor'] : array();
			$t = $fn($t);
			if ($t === null or $t === array()) unset($c['twofactor']);
			else $c['twofactor'] = $t;
			return $c;
		});
	}

	/** Turn two-factor off and forget the secret, its recovery codes and step. */
	static function twoFactorClear()
	{
		return self::aclUpdate(function (array $c) {
			unset($c['twofactor'], $c['twofactorPending']);
			return $c;
		});
	}

	/** The pending (not-yet-confirmed) enrollment secret, base32, or ''. */
	static function twoFactorPendingGet()
	{
		$c = self::aclLoad();
		$p = is_array($c['twofactorPending'] ?? null) ? $c['twofactorPending'] : array();
		return (string) ($p['secret'] ?? '');
	}

	/** Record a pending enrollment secret (replacing any earlier one). */
	static function twoFactorPendingSet($secretBase32)
	{
		return self::aclUpdate(function (array $c) use ($secretBase32) {
			$c['twofactorPending'] = array('secret' => (string) $secretBase32, 'created' => time());
			return $c;
		});
	}

	/** Forget the pending enrollment secret. */
	static function twoFactorPendingClear()
	{
		return self::aclUpdate(function (array $c) {
			unset($c['twofactorPending']);
			return $c;
		});
	}
	...
	/** Record a session. */
	static function sessionPut($token, $expires, $mustChange = false, $twoFactorPending = false)
	{
		if (self::problem() !== null) return false;
		return self::writeJson(self::sessionFile($token), array(
			'expires' => (int) $expires, 'mustChange' => (bool) $mustChange,
			'twoFactorPending' => (bool) $twoFactorPending, 'created' => time()));
	}
	...
	/**
	 * Mark or clear a session's second-factor-pending flag. A session made by
	 * a correct password while two-factor is on is pending until a valid code
	 * (or recovery code) promotes it; until then it may do nothing but present
	 * the code or sign out.
	 */
	static function sessionSetTwoFactorPending($token, $on)
	{
		$s = self::sessionGet($token);
		if ($s === null) return false;
		$s['twoFactorPending'] = (bool) $on;
		return self::writeJson(self::sessionFile($token), $s);
	}
