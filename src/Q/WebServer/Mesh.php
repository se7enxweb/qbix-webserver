...
class Q_WebServer_Mesh
{
	...
	/**
	 * Whether the peer-to-peer mesh is switched on. It is OFF by default: a
	 * default server generates no identity, opens no /Q/sync/* endpoints and
	 * makes no peer connections. The mesh is bootstrapped by an unauthenticated
	 * handshake and then relays requests between servers, so it must be an
	 * explicit choice, not a surface that ships open (compare the cluster-join
	 * hardening).
	 *
	 * Turn it on with Q.webserver.mesh = true in the config, or the environment
	 * variable QBIX_MESH_ENABLED=1.
	 *
	 * @method enabled
	 * @static
	 * @return {boolean}
	...
	{
		$env = getenv('QBIX_MESH_ENABLED');
		if ($env !== false && $env !== '' && $env !== '0') return true;
		if (class_exists('Q_Config', false)) {
			return (bool) Q_Config::get('Q', 'webserver', 'mesh', false);
		}
		return false;
	}
	...
	static function dispatchLocal($httpRequest, $from)
	{
		...
		// A relayed peer request is NOT a local request. Marking it 127.0.0.1
		// would hand an authenticated-but-untrusted mesh peer the whole admin
		// surface (adminAllowed() lets localhost in everywhere), so the client
		// address is a non-local mesh sentinel: /Q/* admin views still demand a
		// real credential over the mesh.
		$parsed['clientIp'] = 'mesh:' . $from;
		$parsed['_remoteAddr'] = 'mesh:' . $from;
		try {
		...
		// Route the response headers through the one serialiser, which drops any
		// header whose name or value carries CR/LF (see the QBX header-injection
		// hardening) — so a mesh-relayed response cannot smuggle header lines.
		$flat = array();
		foreach (($response['headers'] ?? array()) as $k => $v) {
			if (strcasecmp($k, 'Content-Length') === 0) continue;
			$flat[$k] = is_array($v) ? end($v) : $v;
		}
		$out .= Q_WebServer::headerLines($flat);
	...
	{
		// Defence in depth: the /Q/sync/* route is already 404 when the mesh
		// is off, but never process a sync action for a server that has not
		// opted in, whatever road reached here.
		if (!self::enabled()) {
			return array('error' => 'mesh disabled');
		}
		self::init();
		...
		switch ($action) {
			...
			case 'request':
				...
				// Decrypt the request. A peer request is dispatched in-process
				// through the server's own router, so it must be authenticated:
				// only an encrypted payload over an established handshake session
				// is accepted. The old unencrypted `raw` branch let any client
				// that reached the port have an arbitrary request run in-process
				// with no session — an unauthenticated dispatch/SSRF surface —
				// so it is refused.
				$httpRequest = null;
				if (!empty($data['encrypted']) && !empty($data['payload'])) {
					...
					return array('error' => 'Request must be encrypted over an established session');
				}
